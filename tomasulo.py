...
class Execute(object):
	...
	def print_units(self):
		print 'ALU unit: \n\tRS id: %d, result: %f, busy: %r, opcode: %s, vj: %d, vk: %d:' % ( self.alu_unit.rs_id, self.alu_unit.result, self.alu_unit.busy, RESERVE[self.alu_unit.rs_id].opcode, RESERVE[self.alu_unit.rs_id].vj, RESERVE[self.alu_unit.rs_id].vk)
		print "----------------------------------------------------------------"
		print 'MEM unit: \n\tRS id: %d, result: %f, busy: %r, opcode: %s' % ( self.mem_unit.rs_id, self.mem_unit.result, self.mem_unit.busy, RESERVE[self.mem_unit.rs_id].opcode) 
		print "----------------------------------------------------------------"
...
RS_map = {"ALU":(1,3), "MEM":(3,5), "BRANCH":(5,6)}
RESERVE = [RS() for i in range(6)]
...
def clock_tick():
	IF.tick()
	print "IQ LENGTH %d" % (len(IQ.instructions))
	IQ.printQueue()
